...
const Scalegorithm = (function () {
    
    const defaults = {
        mode: "fit", //scale so that child will fit into parent
        parentSize: null, //parent dimensions [0, 0]
        childSize: null, //child dimensions 
        padding: [0, 0], // added horizontal/vertical padding inside parent
        upscaleLimit: 0, // limit upscaling to this factor, 0 for no limit
        verbose: true,
    };

    const calc = function (parms) {
        ...
        if (parms.verbose) {
            ...
            if (parms.parentSize && parms.parentSize.length == 2) {
                if (parms.parentSize[0] <= 0 || parms.parentSize[1] <= 0)
                    throw new Error("scalegorithm parameter invalid: parentSize dimensions must both be > 0");
            ...
            if (parms.childSize && parms.childSize.length == 2) {
                if (parms.childSize[0] <= 0 || parms.childSize[1] <= 0)
                    throw new Error("scalegorithm parameter invalid: childSize dimensions must both be > 0");
            } else throw new Error("scalegorithm parameter invalid: childSize must be of array[2]");

            if (parms.padding && parms.padding.length == 2) {
                if (parms.padding[0] < 0 || parms.padding[1] < 0)
                    throw new Error("scalegorithm parameter invalid: padding dimensions must both be >= 0");
        ...
        const parentWidthPadded = parms.parentSize[0] - parms.padding[0] * 2;
        const parentHeightPadded = parms.parentSize[1] - parms.padding[1] * 2;

        const childScale_W = parms.childSize[0] / parentWidthPadded;
        const childScale_H = parms.childSize[1] / parentHeightPadded;
